...
void set_random_offset(long long *offsets)
{
	long long i;
	long long temp_index;
	long long filesize = file_size_1k*1024;
	long long recordsize = record_size_1k*1024;

	for (i = 0; i<filesize/recordsize; i++)
	{
		if (!i)
		{
			offsets[i] = 0;
		...
		temp_index = genrand64_int64()%i;
		if (temp_index != i) offsets[i] = offsets[temp_index];
		offsets[temp_index] = i*recordsize;
		
		/*offsets[i] = i*recordsize;

		if (!i) continue;

		temp_index = genrand64_int64()%i;
		temp_value = offsets[temp_index];
		offsets[temp_index] = offsets[i];
		offsets[i] = temp_value;*/
	}
	for (i = 0; i<filesize/recordsize; i++)
	{
		printf("%lld\n", offsets[i]);
...
void open_test_files(void)
{
	fp = fopen("io_latency_direct.txt","w");
	...
	/* Open a file */
	fd = open(file_name, O_RDWR | O_DIRECT, 0644);
	if(fd == -1){
		printf("[%s] file %s create fail\n", __FUNCTION__, file_name);
		return;
	}
...
double overwrite_test_files(void)
{
	...
	long long start, end;
        ...
        posix_memalign(&buf, 4096, record_size);

	offsets = (long long*)malloc(sizeof(long long)*(file_size/record_size));

	/*for (i = 0; i<file_size/record_size; i++)
	{
		printf("%lld\n", offsets[i]);
	}*/

	set_random_offset(offsets);
