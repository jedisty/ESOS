...
runCompGcTest()
{
	...
	do
		blktrace -d /dev/sdb -a issue -o ${STATDIR}/result &
		#blktrace -d /dev/sdb -o ${STATDIR}/blktrace_result &
		pid_blktrace=`ps -ef | grep "blktrace" | awk '{print $2}'`
		# ./program  path  file_size_1k  record_size_1k  test_mode
...
#	mkdir ${OUTPUTDIR}
	...
	for TEST_PROG in fsync fdatasync
	do
