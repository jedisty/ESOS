...
//nyg_160408
#ifdef POS_SWAP
//address에 해당하는 pos_swap_bitmap check
//swap entry이면 swap_entry의 값을, 아니면 0 반환
unsigned long pos_get_swap_entry(unsigned long addr)			
{
	struct pos_superblock *sb;
	struct pos_vm_area *vma;
	struct pos_map_array *map_array;
	...
	pages = (addr - pos_vma->vm_start) >> PAGE_SHIFT;
	...
	if (map_array->pfns[index] == 0) {
		...
		if(test_bit(swap_bitmap,index))   // swap_bit=1, swap entry
			return map_array->pfns[index];
		else
...
void pos_set_swap_entry(unsigned long addr, unsigned long pfn)			
{
	struct pos_superblock *sb;
	struct pos_vm_area *vma;
	struct pos_map_array *map_array;
	...
	swap_bitmap = map_array->swap_bitmap;
	if (map_array->pfns[index] == pfn) {
		set_bit(map_array->swap_bitmap,index);
	}else{
		map_array->pfns[index] =pfn;
		set_bit(map_array->swap_bitmap,index);
`	}
