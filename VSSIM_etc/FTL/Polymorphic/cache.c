...
uint32_t cache_select_victim(uint32_t avoid_num)	// sjh
{
	uint32_t idx = clock_hand;

	while(1)
	{
		if (++idx == CACHE_IDX_SIZE)	idx = 0;
...
#ifdef VSSIM_PM
uint32_t cache_evict_page(uint32_t avoid_num)
#else
uint32_t cache_evict_page(uint32_t avoid_num, void *callback)
#endif
{
	...
	uint32_t t_cache_wp = cache_select_victim(avoid_num);		// cache num
	...
	if (cache_idx_table[t_cache_wp].ref_type == PAGE_MAP)
	{
		...
		if (cache_idx_table[t_cache_wp].update_bit)
		{
			...
			old_ppn = evict_rg_node->page_ppn[evict_r_num];
...
//			evict_rg_node->ptr_cache_p[evict_r_num] = NULL;
	...
	else
	{
		...
		if (cache_idx_table[t_cache_wp].update_bit)
		{
			...
			old_ppn = evict_rg_node->extents_ppn;
...
#ifdef VSSIM_PM
	pm_update_region(evict_rg_num);
#else
	pm_update_region(evict_rg_num, callback);
#endif
	return t_cache_wp;
}
