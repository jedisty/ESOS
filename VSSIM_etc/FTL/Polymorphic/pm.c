...
#endif
{
	...
	{
		t_cache_wp = cache_select_victim(-1);
		cache_wp++;
	}
	...
	{
#ifdef VSSIM_PM
		t_cache_wp = cache_evict_page(GET_CACHE_NUM(e_node));
#else
		t_cache_wp = cache_evict_page(GET_CACHE_NUM(e_node),callback);
#endif
	}
...
#endif
{
	...
	for (i=l_num; i<LEAF_NODE_SIZE; i++)
	{
		...
		if (next_l_num < LEAF_NODE_SIZE)					// 2.4		추가
		...
		if ((next_i_num != INDEX_NODE_SIZE) && ((next_l_num == LEAF_NODE_SIZE) || (next_l_num < LEAF_NODE_SIZE && !next_node->used)))
		{
			...
			// 다음 leaf node가 없을 경우
			if (i_node->used == 0 || next_i_node->used == 0)
				break;
			...
			if (cur_start + cur_size >= next_start + next_size)
			{
				...
				next_i_node->size = next_i_node->size - next_node->size;
			...
			else
			{
				...
				next_node->ppn += (cur_node->lpn + cur_node->size - next_node->lpn);
		...
		if (cur_start <= next_start)
		{
			...
			else
			{
				...
				next_node->ppn += (cur_start + cur_size - next_start);					// 2.4 순서변경
		...
		{
#ifdef VSSIM_PM
			printf("Error : It cannot happen in rearrange function!!!\n");
#else
...
void pm_move_forward_lnode(leaf_node *l_node, int l_num)
{
	...
	if (l_num == LEAF_NODE_SIZE-1)
	{
		t_src_node = (leaf_node *)l_node + l_num;
	}
	else
	{
		// move forward one node
...
//			if(rg_node->ptr_cache_p != NULL)
//				printf("page_ppn %d, r_offset %d, rg_num %d, rg_node->ptr_cache_p[r_offset] %d\n", page_ppn, r_offset, rg_num, rg_node->ptr_cache_p[r_offset]);
			...
			{
#ifdef VSSIM_PM
				t_cache_wp = cache_evict_page(-1);
#else
				t_cache_wp = cache_evict_page(-1, callback);
#endif
			}
...
void pm_print_node(extents_node *e_node)
{
	...
	for (i=0; i<ROOT_NODE_SIZE; i++)
	{
		for (j=0; j<INDEX_NODE_SIZE; j++)
		{
			...
			{
				printf("[%s] root[%d] index[%d] used %d lpn %d size %d ppn %d\n", __FUNCTION__, i, j, t_i_node->used, t_i_node->lpn, t_i_node->size, t_i_node->ppn);
				l_node = t_i_node->l_node;
				for (k=0; k<LEAF_NODE_SIZE; k++)
				{
					t_l_node = (leaf_node *)l_node + k;
					if (t_l_node && t_l_node->used)
						printf(" root[%d] index[%d] leaf[%d] used %d lpn %d size %d ppn %d\n", i, j, k, t_l_node->used, t_l_node->lpn, t_l_node->size, t_l_node->ppn);
				}
			}
		}
	}
