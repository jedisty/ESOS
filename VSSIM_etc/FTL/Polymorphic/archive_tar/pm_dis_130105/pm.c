...
/*************************/
static void print_node(extents_node *e_node, uint32_t r_num, uint32_t i_num)
{
	int i, j, k;
	leaf_node *i_node, *l_node;
	leaf_node *t_i_node, *t_l_node;

	ASSERT(e_node, !=, NULL);
	ASSERT(e_node->i_node, !=, NULL);
...
//			if (t_i_node && t_i_node->used)
			{
				printk("[print_node] root[%d] index[%d] used %d lpn %d size %d ppn %d\n", i, j, t_i_node->used, t_i_node->lpn, t_i_node->size, t_i_node->ppn);
...
//					if (t_l_node && t_l_node->used)
						printk("	root[%d] index[%d] leaf[%d] used %d lpn %d size %d ppn %d\n", i, j, k, t_l_node->used, t_l_node->lpn, t_l_node->size, t_l_node->ppn);
				}
			}
		}
	}
...
pm_entry *pm_init_pagemap(uint32_t r_num, extents_node *e_node, void*callback)
{
	...
	pm_update_region(rg_num, callback);
...
void pm_insert_leafnode(uint32_t lpn_ori, uint32_t size, uint32_t new_ppn, extents_node *e_node, void *callback)
{
	...
	printk("[%s] rg_num(%d) r_num(%d/%d) lpn(%d) size(%d) ppn(%d)\n", __FUNCTION__, rg_num, r_num, r_offset, lpn_ori, size, new_ppn);

	if (r_num == 8)
		print_node(e_node, r_num, -1);
...
leaf_node *pm_divide_idxnode(uint32_t r_num, uint32_t i_num, uint32_t l_num, extents_node *e_node, void *callback)		// 2.4
{
	uint32_t r_offset = GET_ROOT_NUM_PER_REGION(r_num);
...
void pm_align_idxnode(uint32_t r_num, uint32_t i_num, uint32_t l_num, extents_node *e_node, leaf_node *l_node, void *callback)	// 2.4
{
	uint32_t cur_start, cur_size, cur_ppn;
	...
	for (i=l_num; i<LEAF_NODE_SIZE; i++)
	{
		...
		if ((next_i_num == INDEX_NODE_SIZE && next_l_num == LEAF_NODE_SIZE) || ((leaf_node *)l_node+i)->used == 0)
			break;
		...
		if ((next_i_num != INDEX_NODE_SIZE) && ((next_l_num == LEAF_NODE_SIZE) || (next_l_num < LEAF_NODE_SIZE && !next_node->used)))
		{
			...
			// 다음 leaf node가 없을 경우
			if (i_node->used == 0 || next_i_node->used == 0)	// sjh
				break;
			...
			if (cur_start + cur_size >= next_start + next_size)
			{
				// 현재 index node의 사이즈와 다음 index node의 lpn, size 변경
				i_node->size = (next_node->lpn + next_node->size) - i_node->lpn;
				next_i_node->lpn = i_node->lpn + i_node->size;
				next_i_node->size = next_i_node->size - next_node->size;		// sjh
				...
				{
					uint32_t t_next_lpn = next_i_node->lpn;
					uint32_t t_next_size = next_i_node->size;

					pm_move_forward_inode ((leaf_node *)r_node->index_node, next_i_num);

					// i_node 사이즈 재조정
					i_node->size = (t_next_lpn + t_next_size) - i_node->lpn;
				}
			...
			else
			{
				...
				next_i_node->size = (next_i_node->lpn + next_i_node->size) - (i_node->lpn + i_node->size);
				next_i_node->lpn = i_node->lpn + i_node->size;
		...
		{
			printk("Error : It cannot happen in rearrange function!!!\n");
		}
	}
}
...
void pm_copy_node(leaf_node *src_i_node, leaf_node *dest_i_node, uint32_t l_num)		// 2.4
{
	...
	if (l_num < LEAF_MID_POS+1)
	{
		...
		dest_i_node->size = (src_i_node->lpn + src_i_node->size) - t_dest_node->lpn;
		dest_i_node->lpn = t_dest_node->lpn;
		src_i_node->size = t_dest_node->lpn - src_i_node->lpn;
	...
	else
	{
		...
		dest_i_node->size = (src_i_node->lpn + src_i_node->size) - (t_src_node->lpn + t_src_node->size);
		dest_i_node->lpn = t_src_node->lpn + t_src_node->size;
		src_i_node->size = (t_src_node->lpn + t_src_node->size) - src_i_node->lpn;
	}
}

void pm_move_back_inode (leaf_node *i_node, int i_num)		// 2.4
{
	...
	for (i=INDEX_NODE_SIZE-2; i>=i_num; i--)
	{
		...
		ASSERT(t_src_inode->l_node, !=, NULL);
		...
		if (t_src_inode->used == 1)
		{
			...
			ASSERT(t_dest_inode->l_node, !=, NULL);
...
void pm_move_forward_inode(leaf_node *i_node, int i_num)	// 2.4
{
	int i;
	leaf_node *t_src_inode = NULL, *t_dest_inode = NULL;

	if (i_num == INDEX_NODE_SIZE-1)
	{
		i = i_num;
		t_src_inode = (leaf_node *)i_node + i_num;
	}
	else
	{
...
{
	int i;
	leaf_node *t_src_lnode = NULL, *t_dest_lnode = NULL;

	if (l_num == LEAF_NODE_SIZE-1)
	{
		t_src_lnode = (leaf_node *)l_node + l_num;
	}
	else
	{
		// move forward one node
		...
		{
			t_src_lnode = (leaf_node *)l_node + (i+1);
			t_dest_lnode = (leaf_node *)l_node + i;

			ASSERT(t_src_lnode, !=, NULL);
			ASSERT(t_dest_lnode, !=, NULL);

			memcpy(t_dest_lnode, t_src_lnode, sizeof(struct leaf_node_));
		
			if (t_src_lnode->used == 0)
				break;
		}
	}
	...
	if (t_src_lnode->used)
	{
		memset (t_src_lnode, 0x00, sizeof(struct leaf_node_));
		t_src_lnode->ppn = -1;
	}
}
...
uint32_t pm_update_region(uint32_t rg_num, void *callback)
{
	...
	if (free_page)
	{
		...
		pm_update_region_idx_ppn(page_num, free_page);
	}
	else
	{
