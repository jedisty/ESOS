...
#ifdef VSSIM_PM
	insert_leaf_node(lpn, size, new_ppn, e_node);
#else
...
#endif
{
	...
	printf("Debug here 1-1 l_node %x\n", l_node);
	...
	for (l_num=0; l_num<LEAF_NODE_SIZE; l_num++)
	{
		...
		printf("Debug here 2-1 %x\n", t_l_node);
		cur_start = t_l_node->lpn;
		cur_size = t_l_node->size;
		cur_ppn = t_l_node->ppn;

		printf("Debug here 2-2\n");
		
		if (t_l_node->used == 0)
		{
			printf("Debug here 3-1\n");
			break;
		}
		// 이전 leaf node에 삽입
		else if (cur_start >= lpn)
		{
			...
			{
				printf("Debug here 3-2\n");
				// ovwrite old leaf node
				break;
			}
			else
			{
				printf("Debug here 3-3\n");
				// if leaf node is full, divide it into halves.
				if (((leaf_node *)l_node + (LEAF_NODE_SIZE-1))->used)
				{
#ifdef VSSIM_PM
					printf("Debug here 4-1\n");
					l_node = divide_leaf_node (r_num, i_num, l_num, e_node);
					printf("Debug here 4-2\n");
#else
					l_node = divide_leaf_node (r_num, i_num, l_num, e_node, callback);
#endif
	...
	{
#ifdef VSSIM_PM
		printf("Debug here 5-1\n");
		l_node = divide_leaf_node (r_num, i_num, l_num, e_node);
		printf("Debug here 5-2\n");
#else
		l_node = divide_leaf_node (r_num, i_num, l_num, e_node, callback);
#endif
		RE_INSERT_POS(l_num, i_num);
...
#ifdef VSSIM_PM
		printf("Debug here 7-2 %u %u %u %x %x\n", r_num, i_num, l_num, e_node, l_node);
		rearrange_index_node(r_num, i_num, l_num, e_node, l_node);
		printf("Debug here 7-3\n");
#else
		rearrange_index_node(r_num, i_num, l_num, e_node, l_node, callback);
#endif
	}
}
...
#ifdef VSSIM_PM
		change_pagemap(r_num, e_node, i_node);
#else
		change_pagemap(r_num, e_node, i_node, callback);
#endif
		return NULL;
	}
	...
	if (l_num != 0)
	{
		...
		if (cur_start + cur_size > next_start)
		{
			...
			else if (cur_start + cur_size > next_start + next_size)
			{
				...
				{
#ifdef VSSIM_PM
					l_node = divide_leaf_node(r_num, i_num, i, e_node);
#else
					l_node = divide_leaf_node(r_num, i_num, i, e_node, callback);
#endif					
...
#endif
{
	...
	// page mapping 영역 초기화
#ifdef VSSIM_PM
	page_map = init_pagemap(r_num, e_node);
#else
	page_map = init_pagemap(r_num, e_node, callback);
#endif
